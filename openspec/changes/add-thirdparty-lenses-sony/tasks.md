...
- [ ] 2.2 Add a data-tier test asserting **every** lens `manufacturer` across all
...
- [ ] 2.3 **Checkpoint:** `npm test` green (no data yet; colour + coverage test
  pass).

## 3. Sigma — DC DN (APS-C) primes & zooms

- [ ] 3.1 Primes: 16mm f/1.4, 18mm f/1.4, 23mm f/1.4, 30mm f/1.4, 56mm f/1.4 DC
  DN — full entries per the per-lens DoD.
- [ ] 3.2 Zooms: 10-18mm f/2.8, 18-50mm f/2.8 DC DN.
- [ ] 3.3 Add ids to a new `── Sigma ──` group in `LENS_DROPDOWN_GROUPS`;
...
- [ ] 4.2 Art primes: 20mm f/1.4, 24mm f/1.4, 35mm f/1.2, 35mm f/1.4, 50mm f/1.2,
