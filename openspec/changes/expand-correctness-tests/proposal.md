...
- Add an **offline link hygiene** suite (Tier 1): every `imageUrl`/`productUrl` parses as a real `https` URL, its host is in an expected-domain allowlist, and `imageUrl` ends in an image extension. The copy-paste guard is **scoped within a brand** and backed by a reviewed allowlist — cross-brand duplicates are legitimate (one maker page/photo serves every mount; 98 exist today). A shared `asin` is likewise legitimate (Amazon parent listings carry mount variants) but the entries sharing it must agree on `manufacturer` and have closely-similar `name`s. ASIN coverage is a **ratchet against a baseline**, not a hard requirement — `buyUrl` no longer exists in the data and a null `asin` degrades to an Amazon search.
